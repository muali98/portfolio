    ...
    return (
        <div className="relative w-full h-screen overflow-hidden">
            ...
            <div className="absolute inset-0 hidden sm:flex">
                <img src={bg_PC} alt="Background for desktop" className="w-full h-full object-cover" />
            </div>
            <div className="absolute inset-0 flex sm:hidden">
                <img src={bg_phone} alt="Background for mobile" className="w-full h-full object-cover" />
            </div>

            {/* Image Carousel with Reduced Width and Conditional Visibility */}
            <div className={`relative flex items-center justify-center h-full space-x-4 w-4/5 mx-auto transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
                {images.map((img, index) => (
                    <div
                        key={index}
                        className={`absolute transition-all duration-700 ease-in-out transform ${getPositionClass(index)}`}
                    >
                        <img
                            src={img}
                            alt={`Slide ${index + 1}`}
